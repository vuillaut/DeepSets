...
class ModelFetcher(object):
    def __init__(self, fname, batch_size, down_sample=10, do_standardize=False, do_augmentation=False):

        self.fname = fname
        self.batch_size = batch_size
        self.down_sample = down_sample

        tab = Table.read(fname, path='angles')
        data = np.transpose([tab[col].data for col in ['theta', 'phi', 'cotheta']])
        data = data.reshape(data.shape[1], data.shape[0], -1)

        n_direction = data.shape[0]
        n_photons = data.shape[1]
        dim = data.shape[2]  # == 3
